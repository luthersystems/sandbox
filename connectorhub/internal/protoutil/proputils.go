...
func CreateChaincodeProposalWithTxIDNonceAndTransient(txid string, typ common.HeaderType, channelID string, cis *peer.ChaincodeInvocationSpec, nonce, creator []byte, transientMap map[string][]byte) (*peer.Proposal, string, error) {
	...
	if err != nil {
		return nil, "", fmt.Errorf("error marshaling ChaincodeHeaderExtension")
	}

	cisBytes, err := proto.Marshal(cis)
	if err != nil {
		return nil, "", fmt.Errorf("error marshaling ChaincodeInvocationSpec")
	}

	ccPropPayload := &peer.ChaincodeProposalPayload{Input: cisBytes, TransientMap: transientMap}
	ccPropPayloadBytes, err := proto.Marshal(ccPropPayload)
	if err != nil {
		return nil, "", fmt.Errorf("error marshaling ChaincodeProposalPayload")
	}
...
func GetBytesProposalResponsePayload(hash []byte, response *peer.Response, result []byte, event []byte, ccid *peer.ChaincodeID) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("error marshaling ChaincodeAction")
	}
...
func GetBytesChaincodeProposalPayload(cpp *peer.ChaincodeProposalPayload) ([]byte, error) {
	...
	return cppBytes, fmt.Errorf("error marshaling ChaincodeProposalPayload: %w", err)
}

// GetBytesResponse gets the bytes of Response
func GetBytesResponse(res *peer.Response) ([]byte, error) {
	resBytes, err := proto.Marshal(res)
	return resBytes, fmt.Errorf("error marshaling Response: %w", err)
}

// GetBytesChaincodeEvent gets the bytes of ChaincodeEvent
func GetBytesChaincodeEvent(event *peer.ChaincodeEvent) ([]byte, error) {
	eventBytes, err := proto.Marshal(event)
	return eventBytes, fmt.Errorf("error marshaling ChaincodeEvent: %w", err)
}
...
func GetBytesChaincodeActionPayload(cap *peer.ChaincodeActionPayload) ([]byte, error) {
	...
	return capBytes, fmt.Errorf("error marshaling ChaincodeActionPayload: %w", err)
}

// GetBytesProposalResponse gets proposal bytes response
func GetBytesProposalResponse(pr *peer.ProposalResponse) ([]byte, error) {
	respBytes, err := proto.Marshal(pr)
	return respBytes, fmt.Errorf("error marshaling ProposalResponse: %w", err)
}
...
func GetBytesHeader(hdr *common.Header) ([]byte, error) {
	...
	return bytes, fmt.Errorf("error marshaling Header: %w", err)
}
...
func GetBytesSignatureHeader(hdr *common.SignatureHeader) ([]byte, error) {
	bytes, err := proto.Marshal(hdr)
	return bytes, fmt.Errorf("error marshaling SignatureHeader: %w", err)
}

// GetBytesTransaction get the bytes of Transaction from the message
func GetBytesTransaction(tx *peer.Transaction) ([]byte, error) {
	bytes, err := proto.Marshal(tx)
	return bytes, fmt.Errorf("error unmarshalling Transaction: %w", err)
}

// GetBytesPayload get the bytes of Payload from the message
func GetBytesPayload(payl *common.Payload) ([]byte, error) {
	bytes, err := proto.Marshal(payl)
	return bytes, fmt.Errorf("error marshaling Payload: %w", err)
}

// GetBytesEnvelope get the bytes of Envelope from the message
func GetBytesEnvelope(env *common.Envelope) ([]byte, error) {
	bytes, err := proto.Marshal(env)
	return bytes, fmt.Errorf("error marshaling Envelope: %w", err)
}
...
func GetActionFromEnvelopeMsg(env *common.Envelope) (*peer.ChaincodeAction, error) {
	...
	if len(tx.Actions) == 0 {
		return nil, fmt.Errorf("at least one TransactionAction required")
	}
...
func createProposalFromCDS(channelID string, msg proto.Message, creator []byte, propType string, args ...[]byte) (*peer.Proposal, string, error) {
	...
	case "upgrade":
		...
		if !ok || cds == nil {
			return nil, "", fmt.Errorf("invalid message for creating lifecycle chaincode proposal")
		}
...
func CheckTxID(txid string, nonce, creator []byte) error {
	...
	if txid != computedTxID {
		return fmt.Errorf("invalid txid. got [%s], expected [%s]", txid, computedTxID)
	}
...
func InvokedChaincodeName(proposalBytes []byte) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("could not unmarshal proposal: %w", err)
	}

	proposalPayload := &peer.ChaincodeProposalPayload{}
	err = proto.Unmarshal(proposal.Payload, proposalPayload)
	if err != nil {
		return "", fmt.Errorf("could not unmarshal chaincode proposal payload: %w", err)
	}
